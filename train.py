...
from mindspore import Model, Tensor, context, save_checkpoint, load_checkpoint, load_param_into_net, Parameter
...
class SPAUCILoss(nn.Cell):
    def __init__(self, alpha=0.5, beta=0.5):
        super(SPAUCILoss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.kappa = 2

        self.a = Parameter(0.5, name='a')
        self.b = Parameter(0.5, name='b')
        self.g = Parameter(1.0, name='g')
        self.s_p = Parameter(0.5, name='s_p')
        self.s_n = Parameter(0.5, name='s_n')
        self.lam_b = Parameter(0.0, name='lam_b')
        self.lam_a = Parameter(0.0, name='lam_a')
    ...
    def construct(self, pred, target):
        ...
        pred_p = pred * ops.reshape(target, (-1, 1))
        pred_n = pred * ops.reshape(1-target, (-1, 1))

        max_val_p = self.log(1 + self.exp(self.kappa * (self.square(pred_p - self.a) - \
                                                    2 * (1 + self.g) * pred_p - self.s_p))) / self.kappa
        max_val_n = self.log(1 + self.exp(self.kappa * (self.square(pred_n - self.b) + \
                                                    2 * (1 + self.g) * pred_n - self.s_n))) / self.kappa
        res = self.s_p + self.mean(max_val_p) / self.alpha + \
                self.s_n + self.mean(max_val_n) / self.beta - \
                1 * self.square(self.g) - self.lam_b * (self.b - 1 - self.g) + self.lam_a * (self.a + self.g)
